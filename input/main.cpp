  ...
  for (;;) {
    ...
    if (int fingers = controller->ReadFingers(&f)) {
      for (int i = 0; i < fingers; ++i) {
        ...
        scene->DrawRectangle(850 + fx, 318 + fy, 10, 10,
    ...
    scene->DrawRectangle(
        20, MAX(0, MIN(FRAME_HEIGHT, (FRAME_HEIGHT / 2) - q.x * 40)), 15, 10,
        Color{0xFF, 0x00, 0x00});
    scene->DrawRectangle(
        40, MAX(0, MIN(FRAME_HEIGHT, (FRAME_HEIGHT / 2) - q.y * 40)), 15, 10,
        Color{0xFF, 0x00, 0x00});
    scene->DrawRectangle(
        60, MAX(0, MIN(FRAME_HEIGHT, (FRAME_HEIGHT / 2) - q.z * 40)), 15, 10,
        Color{0xFF, 0x00, 0x00});
    scene->DrawRectangle(
        80, MAX(0, MIN(FRAME_HEIGHT, (FRAME_HEIGHT / 2) - q.w * 40)), 15, 10,
        Color{0xFF, 0x00, 0x00});
